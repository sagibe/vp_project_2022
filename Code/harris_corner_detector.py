...
K = 0.05
CHECKERBOARD_THRESHOLD = 20 #1e1
GIRAFFE_THRESHOLD = 1e5 #1e6
...
def create_grad_x_and_grad_y(
    ...
    Img_shiftx=input_image[:,:-1]
    Img_shiftx =np.insert(Img_shiftx,0,0,axis=1)
    Ix = input_image-Img_shiftx
    Ix[:,0] = np.zeros(h)
    # Ix = Ix[:,1:]

    Img_shifty = input_image[:-1, :]
    Img_shifty = np.insert(Img_shifty, 0, 0, axis=0)
    Iy = input_image-Img_shifty
    Ix[0, :] = np.zeros(w)
    # Iy = Ix[1:,:]
    return Ix, Iy
...
def our_harris_corner_detector(input_image: np.ndarray, K: float,
    ...
    Recipe:
    (1) calculate the image from the input image and the parameter K.
    ...
    input_image = input_image[:h-h_rem,:w-w_rem]

    response_image = calculate_response_image(input_image, K)
    res_tiles = black_and_white_image_to_tiles(response_image,tile_size,tile_size)
    res_tiles_nms=res_tiles.copy()*0
    t,h,w=res_tiles_nms.shape
    for i in range(t):
        indmax=np.argmax(res_tiles[i,:,:])
        coor = np.unravel_index(indmax,(tile_size,tile_size))
        res_tiles_nms[i,coor[0],coor[1]]=res_tiles[i,coor[0],coor[1]]

    mat=image_tiles_to_black_and_white_image(res_tiles_nms,input_image.shape[0],input_image.shape[1])
    output_image = np.zeros((input_image.shape[0],input_image.shape[1]))
    output_image[mat>threshold]=1
    output_image = np.pad(output_image, [(0, h_rem), (0, w_rem)], mode='constant')
